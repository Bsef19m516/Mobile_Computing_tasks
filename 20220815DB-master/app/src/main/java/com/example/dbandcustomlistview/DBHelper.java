...
public class DBHelper extends SQLiteOpenHelper {
    ...
    //can make it have  a toast alert on adding
    ...
    public void  updateStudent(StudentModel STUDENTModel){
        ...
        cv.put(STUDENT_ROLL, STUDENTModel.getRollNmber());
        cv.put(STUDENT_ENROLL, STUDENTModel.isEnroll());
        Cursor cursor = db.rawQuery("SELECT * FROM " +  STUDENT_TABLE + " WHERE " + STUDENT_NAME + "=?" , new String[] {STUDENTModel.getName()});
    ...
    public void  deleteStudent(StudentModel STUDENTModel){
        ...
        if(cursor.getCount()>0)
        {
            db.delete(STUDENT_TABLE, STUDENT_NAME + "=?", new String[] {STUDENTModel.getName()});
        }
        db.close();
    }
